...
export default function MockGameSimulation() {
  ...
  // ===== Refs to keep executeTurn stable =====
  ...
  const handleUseCard = (card) => {
    setPlayers(prevPlayers => {
      const updated = prevPlayers.map(p => ({ ...p, cards: [...p.cards], flags: { ...(p.flags||{}) } }));
      const p = updated[activeIdxRef.current];
      const eff = applyCardEffect(p, card, updated);
  ...
  const executeTurn = useCallback(() => {
    ...
    setTimeout(() => {
      setPlayers(prev => {
        ...
        // Bot uses a card if low on cash
        ...
        // Extra roll: don't advance player
