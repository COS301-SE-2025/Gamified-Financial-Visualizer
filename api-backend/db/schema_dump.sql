-- ========================================
-- Gamified Financial Visualizer Schema (PostgreSQL) changed
-- ========================================
...
-- VISUAL ASSETS (Unlockable AR world themes)
CREATE TABLE visual_assets (
    asset_id SERIAL PRIMARY KEY,
    user_id INT NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    asset_theme VARCHAR(50) NOT NULL CHECK (
        asset_theme IN (
            'classic_day',
            'sunset_pink',
            'rainy_evening',
...
CREATE TABLE learning_modules (
    ...
    banner_image_path VARCHAR(255) NOT NULL DEFAULT 'banners/default_module.png'
);
...
CREATE TABLE achievements (
    ...
    achievement_type VARCHAR(50) NOT NULL CHECK (
        achievement_type IN ('goal', 'quiz', 'challenge', 'transaction', 'milestone', 'tutorial', 'misc', 'budget', 'ar')
    ),
    ...
    banner_image_path VARCHAR(255) -- For umbrella achievements only
);
...
CREATE TABLE post_community_tags (
...
CREATE INDEX idx_post_tags_community_id ON post_community_tags(community_id);
