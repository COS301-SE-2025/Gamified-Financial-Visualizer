...
describe('Auth API Integration Tests', () => {
  ...
  describe('PUT /api/auth/:userId/update-profile', () => {
    const updatedUser = {
      user_id: 1,
    ...
    it('should update user profile with valid data', async () => {
      ...
      const response = await request(app)
        .put('/api/auth/1/update-profile')
        .send({ username: 'updateduser', full_name: 'Updated User' });
      ...
      expect(logger.info).toHaveBeenCalledWith(
        '[Auth] Updated profile for user ID 1'
      );
    });
    ...
    it('should return 500 when update fails', async () => {
      (userService.updateUserProfile as jest.Mock).mockRejectedValueOnce(new Error('DB Error'));
      ...
      expect(logger.error).toHaveBeenCalledWith(
        '[Auth] Failed to update profile for user ID 1:',
        expect.any(Error)
      );
    });
  });
