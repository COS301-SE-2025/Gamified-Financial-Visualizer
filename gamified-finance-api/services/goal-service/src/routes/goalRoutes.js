...
router.post('/', async (req, res) => {
    ...
    try {
        const goal = await (0, goalService_1.createPersonalGoal)({
            user_id,
            goal_name,
            goal_type,
            target_amount,
        ...
        logger_1.logger.error('[Routes] Failed to create goal', error);
...
router.post('/community', async (req, res) => {
    const { user_id, community_id, goal_name, goal_type, target_amount, target_date, goal_status } = req.body;
    if (!user_id || !community_id || !goal_name || !goal_type ||
    ...
    try {
        const goal = await (0, goalService_1.createCommunityGoal)({
            user_id,
            community_id,
            goal_name,
            goal_type,
        ...
        logger_1.logger.error('[Routes] Failed to create community goal', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
/**
...
router.get('/user/:userId', async (req, res) => {
    ...
    try {
        const goals = await (0, goalService_1.getPersonalGoalsByUserId)(userId);
        res.status(200).json({
            status: 'success',
            data: goals
        });
        ...
        logger_1.logger.error('[Routes] Failed to fetch personal goals', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
/**
 ...
 */
router.get('/user/:userId/community', async (req, res) => {
    const { userId } = req.params;
    try {
        const goals = await (0, goalService_1.getCommunityGoalsForUser)(userId);
        res.status(200).json({
            status: 'success',
            data: goals
        });
        ...
        logger_1.logger.error('[Routes] Failed to fetch community goals for user', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
/**
...
router.put('/:goalId', async (req, res) => {
    ...
    try {
        const updated = await (0, goalService_1.updateGoalNameByUser)(goalId, user_id, goal_name.trim());
        if (!updated) {
            res.status(404).json({
                status: 'error',
        ...
        logger_1.logger.error('[Routes] Failed to update goal name', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
/**
...
router.delete('/:goalId', async (req, res) => {
    ...
    try {
        const deleted = await (0, goalService_1.deleteGoalByUser)(goalId, user_id);
        if (!deleted) {
            res.status(404).json({
                status: 'error',
        ...
        logger_1.logger.error('[Routes] Failed to delete goal', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
/**
 ...
 */
router.put('/community/:goalId', async (req, res) => {
    const { goalId } = req.params;
    const { user_id, goal_name } = req.body;
    if (!user_id || !goal_name || goal_name.trim() === '') {
        res.status(400).json({
    ...
    try {
        const updated = await (0, goalService_1.updateCommunityGoalNameByUser)(goalId, user_id, goal_name.trim());
        if (!updated) {
            res.status(404).json({
                status: 'error',
        ...
        logger_1.logger.error('[Routes] Failed to update community goal name', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
/**
 ...
 * @body {string} user_id - The ID of the user requesting deletion
 */
router.delete('/community/:goalId', async (req, res) => {
    const { goalId } = req.params;
    const { user_id } = req.body;
    if (!user_id) {
        res.status(400).json({
    ...
    try {
        const deleted = await (0, goalService_1.deleteCommunityGoalByUser)(goalId, user_id);
        if (!deleted) {
            res.status(404).json({
                status: 'error',
                message: 'Community goal not found or user not authorized.'
        ...
        logger_1.logger.error('[Routes] Failed to delete community goal', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
/**
 ...
 */
router.get('/user/:userId/in-progress', async (req, res) => {
    const { userId } = req.params;
    try {
        const goals = await (0, goalService_1.getInProgressPersonalGoals)(userId);
        res.status(200).json({ status: 'success', data: goals });
    }
    catch (error) {
        logger_1.logger.error('[Routes] Failed to fetch in-progress goals', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
/**
 ...
 */
router.get('/user/:userId/achieved', async (req, res) => {
    const { userId } = req.params;
    try {
        const goals = await (0, goalService_1.getAchievedPersonalGoals)(userId);
        res.status(200).json({ status: 'success', data: goals });
    }
    catch (error) {
        logger_1.logger.error('[Routes] Failed to fetch achieved goals', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
/**
 ...
 */
router.get('/user/:userId/community/achieved', async (req, res) => {
    const { userId } = req.params;
    try {
        const goals = await (0, goalService_1.getAchievedCommunityGoals)(userId);
        res.status(200).json({ status: 'success', data: goals });
    }
    catch (error) {
        logger_1.logger.error('[Routes] Failed to fetch achieved community goals', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
/**
 ...
 */
router.get('/user/:userId/community/in-progress', async (req, res) => {
    const { userId } = req.params;
    try {
        const goals = await (0, goalService_1.getInProgressCommunityGoals)(userId);
        res.status(200).json({ status: 'success', data: goals });
    }
    catch (error) {
        logger_1.logger.error('[Routes] Failed to fetch in-progress community goals', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
/**
 ...
 */
router.get('/community/:goalId', async (req, res) => {
    const { goalId } = req.params;
    try {
        const goal = await (0, goalService_1.getCommunityGoalById)(goalId);
        if (!goal) {
            res.status(404).json({
                status: 'error',
        ...
        logger_1.logger.error('[Routes] Failed to fetch community goal', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
/**
 ...
 */
router.get('/personal/:goalId', async (req, res) => {
    const { goalId } = req.params;
    try {
        const goal = await (0, goalService_1.getPersonalGoalById)(goalId);
        if (!goal) {
            res.status(404).json({
                status: 'error',
        ...
        logger_1.logger.error('[Routes] Failed to fetch personal goal', error);
        res.status(500).json({ status: 'error', message: 'Internal server error' });
    }
});
